...
class DIPControllerValidationTest extends MapperBase {
    ...
    @Test
    void shouldGiveValidationErrorWhenCreateDipApplicationWhenSpecifyEmptyIncomeItemsList() throws Exception {
        //Given
        var dipApplicationRequest = getDIPApplicationRequestDto();
        var applicant = dipApplicationRequest.getApplicants().get(0);
        applicant.getIncome().setIncome(Collections.emptyList());
        ...
        mockMvc.perform(post("/application/dip").with(csrf()).content(objectMapper.writeValueAsString(dipApplicationRequest))
                ...
                .andExpect(jsonPath("$.violations[0].message").value("at least one income must be specified"));
    }
